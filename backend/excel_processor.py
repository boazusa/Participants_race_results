...
class BestResultsFromExcel:
    ...
    def load_participants_from_excel(self):
        ...
        if "name" in df.columns:
            ...
            safe_print("❌ 'name' column not found after rename. Check column names.")
            return None
        ...
        safe_print(f"✅ Loaded {len(df)} participants from Excel.")
        return df
    ...
    def filter_participants(
        ...
        safe_print(f"✅ Filtered to {len(self.filtered_names)} participants.")
    ...
    def fetch_best_result(self, first_name, last_name, category, timeout_seconds=10):
        ...
        try:
            ...
            safe_print(f"❌ Request failed for {first_name} {last_name}: {e}")
            return None

        soup = BeautifulSoup(response.text, "html.parser")
        table = soup.find("table")
        if not table:
            safe_print(f"❌ No table found for {first_name} {last_name}")
            return None
        ...
        if not rows:
            safe_print(f"❌ No results for {first_name} {last_name}")
            return None
        ...
        if df_cat.empty:
            safe_print(f"⚠️ No results for {first_name} {last_name} in {category}")
            return None
    ...
    def get_best_results_for_category(self, category):
        ...
        if not self.filtered_names:
            safe_print("❌ No filtered names available. Run filter_participants() first.")
            return None
        ...
        if not best_results:
            safe_print("❌ No best results found.")
            return None
        ...
        if self.output_excel_path:
            ...
            safe_print(f"✅ Saved best results to {self.output_excel_path}")
