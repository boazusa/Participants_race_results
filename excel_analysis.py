...
class BestResultsFromExcel:
    ...
    def normalize_distance(value):
        ...
        if s in ["10 ק\"מ", "10ק\"מ", "9800", "10000", "10 קמ"]:
            return "10K"
        if s in ["21097", "21 ק\"מ", "21000", "21K", "21k", "חצי מרתון", "חצי מרתון תחרותי", "חצי-מרתון", "חצי_מרתון", "Half Marathon"]:  # Added "Half Marathon"
            return "21K"
    ...
    def load_participants_from_excel(self):
        ...
        df['מגדר'] = df['מגדר'].astype(str).str.strip().replace({
            'ז': 'male', 'נ': 'female', 'זכר': 'male', 'נקבה': 'female',
            'm': 'male', 'f': 'female', 'M': 'male', 'F': 'female'  # Added uppercase
        }).str.lower()
    ...
    def filter_participants(self, min_year=None, max_year=None, gender=None, race_keyword=None):
        ...
        if race_keyword is not None:
            ...
            if not mask.any() and race_keyword in ["5K", "10K", "15K", "Half Marathon", "21K", "42K"]:  # Added '21'
                if race_keyword == "21" or race_keyword == "Half Marathon":
                    mask = df['מקצה'].apply(lambda x: str(self.normalize_distance(str(x)))) == "21K"
                else:
        ...
        self.filtered_names = list(df[['שם פרטי', 'שם משפחה']].itertuples(index=False, name=None))
...
if __name__ == "__main__":
    ...
    processor = BestResultsFromExcel(excel_input_path=excel_input, race_name="Your Race", output_excel_path=output_excel)
    ...
    filtered = processor.filter_participants(min_year=1970, max_year=2005, gender='male', race_keyword='21K')  # Changed to '21K'
