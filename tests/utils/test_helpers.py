...
class DataFrameAssertions:
    ...
    def assert_age_range(df, birth_year_col, min_age, max_age, current_year=None):
        ...
        if birth_year_col in df.columns:
            df_copy = df.copy()
            df_copy[birth_year_col] = pd.to_numeric(
                df_copy[birth_year_col], errors="coerce"
            )
    ...
    def assert_gender_filter(df, gender_col, expected_gender):
        ...
        if gender_col in df.columns:
            ...
            assert (
                len(filtered_df) > 0
            ), f"No participants with gender '{expected_gender}'"

            # Check if any other genders are present
            other_genders = df[df[gender_col] != expected_gender]
            if len(other_genders) > 0:
                print(
                    f"Warning: {len(other_genders)} participants with different gender"
                )
...
class HTTPMockHelpers:
    ...
    def create_mock_response(
        status_code=200, text="", headers=None, raise_for_status=None
    ):
...
class PerformanceHelpers:
    ...
    def assert_execution_time(execution_time, max_seconds):
        ...
        ), f"Execution time {execution_time}s exceeds maximum {max_seconds}s"
    ...
    def assert_memory_usage(memory_used_mb, max_mb):
        ...
        assert (
            memory_used_mb <= max_mb
        ), f"Memory usage {memory_used_mb}MB exceeds maximum {max_mb}MB"
...
def __enter__(self):
    self.file_path, self.temp_dir = FileSystemHelpers.create_temp_excel_file(
...
def assert_dataframe_equals(
    actual, expected, check_dtype=True, check_column_order=False
):
